...
CSV_PATH = BASE_DIR / "DOHMH_New_York_City_Restaurant_Inspection_Results_20260416.csv"
GEOJSON_PATH = BASE_DIR / "Borough_Boundaries_20260416.geojson"
...
with col_right:
    with st.container(border=True):
        ...
        st.caption(
            "Overall, grades are similarly distributed across the boroughs, with Grade A being "
            "the large majority in every borough. The Bronx and Manhattan have slightly higher "
            "percentages of Grade A restaurants, Queens has the largest percentage of Grade B, "
            "and Staten Island has the largest proportion of Grade C."
        )
...
with st.container(border=True):
    ...
    st.markdown(
        """
Average inspection scores and critical violation rates both vary across NYC boroughs, and
Staten Island stands as the only borough high on both metrics. Since higher inspection scores
and higher critical violation rates indicate worse outcomes, this suggests relatively weaker
inspection performance in Staten Island.

Queens also shows a high average inspection score, and the Bronx has the lowest average
inspection score, but the Bronx still shows relatively high critical violation rates. Overall,
the borough rankings differ slightly across the metrics used, but Staten Island remains at one
of the weaker ends of its distribution.
"""
    )