...
def build_sentiment_hist(ta):
    fig = px.histogram(
        ...
        title="Average Review Sentiment by Top Review Categories (-1 to +1)",
    ...
    fig.update_layout(
        height=470,
        xaxis_title="Average VADER Sentiment (-1 to +1)",
...
def sentiment_takeaway(ta):
    return (
        f"Public review sentiment is generally positive. The average VADER compound score is "
        f"**{ta['sentiment'].mean():.3f}** on a scale from **-1 to +1**, where **-1** means very negative, "
        f"**0** means neutral, and **+1** means very positive. Strongly negative reviews are relatively rare."
    )
...
def wordcloud_takeaway():
    return (
        "The sentiment expressed in Grade A and Grade B/C reviews is broadly similar. "
        "Both word clouds are dominated by positive terms such as **great**, **good**, "
        "**delicious**, **best**, and **nice**, suggesting that diners often describe "
        "restaurants positively regardless of inspection grade. The main difference is "
        "that Grade B/C reviews include slightly more discovery- and value-oriented words "
        "such as **find**, **gem**, **hidden**, and **new**, but the overall sentiment remains "
        "positive rather than clearly negative."
    )
